...
ПРАВИЛА ИЗВЛЕЧЕНИЯ:
1. ИГНОРИРУЙ служебные строки переноса: «Итого по странице», «Промежуточный итог», «Всего перенесено», «Страница X из Y», «Лист X». Не включай их в массив items!
2. В таблице УПД перед количеством идет колонка 2 "Код единицы измерения" (796, 778, 166, 112). КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО брать их как количество! Настоящее количество — в графе "Количество (объем)" (1.000, 2.000 и т.д.).
3. Вычисляй ai_multiplier (коэффициент перевода в базовые единицы кг/л/шт):
   - Консервы в банках (шт): сухой вес без рассола (сух/н) в кг (272гр -> 0.272).
   - Весовые товары в граммах (500г, 454г) -> 0.5, 0.454.
   - Если товар УЖЕ в кг/л и количество дробное -> ai_multiplier = 1.0.
   - Если единица измерения "шт/уп", а в названии фасовка (Масло 5л, Соус 5.4кг) -> ai_multiplier = 5.0, 5.4.
   - Листовые товары (нори 100л) -> 100.0. Чай в пакетиках (100 пак) -> 100.0. Грибы шимиджи -> 0.15.
4. МИКРО-ЯКОРЬ original_prefix: скопируй строго первое слово из названия товара пиксель-в-пиксель со скана.
5. ЧИСТАЯ КАТЕГОРИЯ clean_category: строго одна из ["Мясо и птица", "Рыба и морепродукты", "Овощи и фрукты", "Молочные продукты", "Бакалея", "Консервы", "Напитки", "Хозяйственные товары", "Прочее", "Без категории"].
6. ЕСЛИ НА ЭТОЙ СТРАНИЦЕ ЕСТЬ ПОДВАЛ ДОКУМЕНТА («Всего к оплате» / «Итого с НДС»): извлеки это число в поле doc_printed_total_sum. Если подвала на этой странице нет — верни 0.0.
