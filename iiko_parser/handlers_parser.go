...
func handleImport(w http.ResponseWriter, r *http.Request) {
	...
	if len(invoiceItems) == 0 {
		http.Error(w, "В накладной нет сопоставленных позиций для отправки", http.StatusBadRequest)
		return
	}
	...
	for _, item := range req.Items {
		if item.MappedUUID != "" {
			...
			if medianPrice.Valid && medianPrice.Float64 > 0 && pricePerUnit > medianPrice.Float64*1.15 && item.Sum > 500 {
				...
				go func(cID int, pName string, growthPct, mPrice, cPrice float64, vName, iNum string) {
					var compName string
				...
				}(companyID, prodName, growth, med, cur, vendor, invNum)
			}
