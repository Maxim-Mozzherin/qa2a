...
func (s *IikoService) SaveSettings(companyID, userID int, host, login, pass string) error {
	...
	if role != "owner" && role != "manager" {
		return fmt.Errorf("только Владелец или Управляющий может изменять параметры интеграции iiko RMS")
	}
...
func (s *IikoService) GetSettings(companyID, userID int) (*models.IikoSettings, error) {
	...
	role := strings.ToLower(member.Role)
	if role != "owner" && role != "manager" {
		return nil, fmt.Errorf("доступ запрещен: просматривать настройки может только Владелец или Управляющий")
	}
	...
	if err != nil {
		...
		for key, items := range storeGroups {
			...
			for i := 0; i < len(items); i += chunkSize {
				...
				res, err := s.httpClient.Do(req)
				if err != nil {
					log.Printf("ошибка сети при выгрузке списания: %v\n", err)
					continue
				}
				respBody, _ := io.ReadAll(res.Body)
				res.Body.Close()
				...
				if res.StatusCode != http.StatusOK {
					log.Printf("iiko отклонил списание (код %d): %s\n", res.StatusCode, string(respBody))
					continue
				}
				...
				if err := json.Unmarshal(respBody, &iikoResult); err == nil {
					if strings.ToUpper(iikoResult.Result) == "ERROR" {
						log.Printf("ошибка валидации списания iiko: %s\n", strings.Join(iikoResult.Errors, "; "))
						continue
					}
				}
