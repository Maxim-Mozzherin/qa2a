            ...
            try {
                ...
                if (Array.isArray(paths) && paths.length > 0) {
                    ...
                    const items = paths.map(p => {
                        ...
                        if (isVideo) {
                            return `<video src="${fullUrl}" controls style="max-height: 120px; max-width: 140px; border-radius: 8px; border: 1px solid var(--border);"></video>`;
                        } else {
                            return `<a href="${fullUrl}" target="_blank"><img src="${fullUrl}" style="max-height: 100px; max-width: 120px; object-fit: cover; border-radius: 8px; border: 1px solid var(--border);"></a>`;
                        }
                    }).join('');
...
function addToOfflineQueue(url, method, payload) {
    ...
    if (method === 'PUT') {
        ...
        if (existingIdx !== -1) {
            ...
            queue.push({ url, method, payload, companyId });
        }
    } else {
        queue.push({ url, method, payload, companyId });
    }
    ...
    while (queue.length > 0) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                queue.shift(); // Drop strictly invalid client requests, keep 408/429 for retry
