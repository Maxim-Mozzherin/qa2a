...
func main() {
	...
	h := handlers.New(authSvc, invSvc, repSvc, iikoSvc, mktSvc, cfg.BotToken)
	...
	uploadFS := neuteredFileSystem{fs: http.Dir("uploads")}
	r.PathPrefix("/uploads/").Handler(http.StripPrefix("/uploads/", http.FileServer(uploadFS)))
